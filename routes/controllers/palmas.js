...
const InfoSalon = async (req, res) => {
  ...
  try {
    ...
    const db = client.db('EdificioPalmas');
    const salon = await db.collection('palmas').findOne({ salon: datos.id });

    if (salon) {
      res.json({ status: "ok", salon });
    } else {
